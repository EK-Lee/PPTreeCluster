#' @title Projection Pursuit Cluster Tree Plot
#' @description Draw projection pursuit cluster tree with tree structure. It is
#' modified from a function in party library.
#' @param x PPtreecluster object
...
plot.PPtreecluster<-function(x,
  ...
  plotPPtree<-function(PPtreeobj,node.id,xlim,ylim){
    ...
    if(TS[node.id,2]==0){
      x<-xlim[1]+0.5
      y<-ylim[2]-1
      Final.Node.V<-grid::viewport(x=unit(x,"native"),
                             y=unit(y,"native"),
                             width=unit(1,"native"),
    ...
    node.V<-grid::viewport(x=unit(x0,"native"),
                     y=unit(y0,"native"),
                     width=unit(1,"native"),
    ...
    LeftEdge.V<-grid::viewport(x=unit(xlpos,"native"),
                         y=unit(ylpos,"native"),
    ...
    upViewport()
    RightEdge.V<-grid::viewport(x=unit(xrpos,"native"),
                          y=unit(yrpos,"native"),
                          width=unit(xlpos-xrpos,"native"),
                          height= unit(1,"lines"))
  ...
  node.inner.PPtree<-function(PPtreeobj,node.id){
    ...
    Inner.Node.V<-grid::viewport(x=unit(0.5,"npc"),
                           y=unit(0.5,"npc"),
    ...
    Inner.Node.Id.V<-grid::viewport(x=unit(0.5,"npc"),
                              y=unit(1,"npc"),
  ...
  node.terminal.PPtree<- function(PPtreeobj,node.id){
    ...
    Terminal.Node.V<-grid::viewport(x=unit(0.5,"npc"),
                              y=unit(0.8,"npc"),
    ...
    Terminal.Node.Id.V<-grid::viewport(x=unit(0.5,"npc"),
                                 y=unit(1,"npc"),
                                 width=max(unit(1,"lines"),
                                           unit(1.2,"strwidth",
                                                as.character(node.id))),
  ...
  nx<-PPtreeobj$ncluster
  ny<-calc.depth(PPtreeobj)
  tnex<-1
  node.id<-1
  grid::grid.newpage()
  PPtree.Main.V<-grid::viewport(layout=grid.layout(3,3,
